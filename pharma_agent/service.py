...
def search_company_jobs(company_name: str, location: str, api_key: str) -> list[JobRecord]:
    query = f'{company_name} jobs {location}'
    try:
    ...
    *,
    ...
    if mode == "sample":
        ...
        if progress_callback:
            progress_callback("search", "active", f"Searching pharmacy companies for {query} across {len(queries)} query variations.")
        ...
        for index, current_query in enumerate(queries, start=1):
            ...
            if progress_callback:
                progress_callback("search", "active", f"Search variation {index} of {len(queries)} completed.")
        ...
        if progress_callback:
            ...
            progress_callback("dedupe", "active", "Merging duplicate companies from multiple query variations.")
        ...
        if progress_callback:
            ...
            progress_callback("report", "active", "Finding related job openings for each company and building the Word report.")
